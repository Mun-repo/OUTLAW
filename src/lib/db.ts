...
async function createPgliteSql(): Promise<Sql> {
  ...
  // files are tracked in `_migrations` (including `0006_orders.sql`). The glob does not descend, so the opt-in
 ...
 * New catalog columns (events feed, product galleries) live in 0004.
 * Pickup orders (Stripe boutique) live in 0006.
 */
